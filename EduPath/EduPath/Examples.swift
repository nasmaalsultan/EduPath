...
struct Examples: View {
        ...
        NavigationView {
            ZStack(alignment: .top) {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    VStack(spacing: 20) {
                        NavigationLink(destination: UKPersonalStatement()) {
                            exampleButton(title: "Personal Statement: UK Russel Group")
                        }

                        NavigationLink(destination: IvySupplementals()) {
                            exampleButton(title: "Ivy League: Supplement Essays")
                        }

                        NavigationLink(destination: T50AppEssays()) {
                            exampleButton(title: "T50 USA: Application Essays")
                        }

                        NavigationLink(destination: CanadaPersonalStatement()) {
                            exampleButton(title: "Canada: Personal Statements")
                        }
                    }
                    .padding(.horizontal)
    ...
    //Reusable button
    func exampleButton(title: String) -> some View {
        Text(title)
            .fontWeight(.semibold)
            .frame(maxWidth: .infinity)
            .padding()
